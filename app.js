/**
 * 数字易经 - 占卜工具
 * Digital I Ching Divination Tool
 */

let count = 0;
let results = [];
let isTossing = false; // 防连点锁
 ...
 */
function showResult() {
    let code = results.join('');
    let resultData = guaData[code];

    document.getElementById('final-target').innerText = "所求之事：" + (document.getElementById('target').value || "未填写");
    ...
    if (resultData) {
        ...
        document.getElementById('gua-text').innerText = resultData.text;
    } else {
        document.getElementById('gua-name').innerText = code;
        document.getElementById('gua-text').innerHTML = '<span class="not-found">未找到对应卦象数据（' + code + '）</span>';
    }
